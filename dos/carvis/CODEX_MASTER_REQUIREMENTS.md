...
## 0. 当前状态覆盖说明（2026-07-09）

本文件下方保留了早期目标和阶段规则；当前实现已经进入可运行 MVP 后的文档漂移修正与质量加固阶段。若早期段落与本节冲突，以本节和当前代码为准。

当前部署事实：

- 部署目标：macOS，分支 `macos-deploy`（基线 `backup/mvp-nixos-20260702-020835`）。
- NixOS 远端版本保留在 `backup/mvp-nixos-*` 历史分支中。
- 启动方式：三进程依次启动 — messagebus（TCP socket）→ agentruntime → electron。
- API Key 通过项目根目录 `keys.txt` 注入（已在 .gitignore）。
...
- 当前 provider routing：manager/writer/engineer 走 DeepSeek OpenAI-compatible；artist/researcher 走 Qwen OpenAI-compatible；artist 可通过本地 artist-image MCP wrapper 生成图片资产。
- Claude Code 调用方式：`@anthropic-ai/claude-agent-sdk` warm SDK（替代 CLI spawn）。
- 正式 workplace 路径：`workplaces/runs/<timestamp-request>/<role>/`。
- systemd 服务、NAS 模块已在 macOS 部署中移除。
